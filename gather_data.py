...
def fetch_google_image(lat, long, zoom=12, size="1000x500"):
    ...
    API_KEY = 'AIzaSyDqu32LSH52LcMq-Bg9N4hBZQ6Bd7lE2dU'
    url = (API_ENDPOINT +
...
def fetch_images_from_coordinates(service="mapbox"):
    for (lat, lon), country in coordinates:
        time.sleep(0.1)
        if service == "mapbox":
            image_data = fetch_mapbox_image(lat=lat, lon=lon)
        elif service == "google":
...
countries = list(COUNTRY_CENTER_COORDS.keys())
coordinates = []
for country in countries:
    # time.sleep(0.2)
    coordinates.append((get_capital_coordinates(country), country))
    # if country in LARGEST_COUNTRIES:
    #     coordinates.append((random_coordinate_from_country(country,world_data), country))
    print(coordinates)
# 2. Loop over the generated coordinates and retrieve images

fetch_images_from_coordinates('google')