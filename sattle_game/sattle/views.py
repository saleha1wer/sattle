...
def calculate_distance(image, guessed_country):
    coords_image = (float(image.coordinates.split(',')[0]), float(image.coordinates.split(',')[1]))

    # Fetching the center coordinates of the guessed country
    coords_guessed_country = COUNTRY_CENTER_COORDS.get(guessed_country, (0, 0))

    # Calculate distance using geopy
    distance = geodesic(coords_image, coords_guessed_country).kilometers

    # Calculate direction using the previously provided function
    direction = calculate_direction(coords_image[0], coords_image[1], coords_guessed_country[0], coords_guessed_country[1])
