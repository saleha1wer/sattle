...
def calculate_distance(image, guessed_country):
    ...
    return distance,correct  # returns distance in kilometers
...
def home(request):
    ...
    # Check if there's an existing correct guess for this image
    existing_correct_guess = Guess.objects.filter(image=random_image, distance=0).exists()

    context = {
        'image_url': random_image,
        'existing_correct_guess': existing_correct_guess,
...
def submit_guess(request):
    if request.method == 'POST':
        ...
        distance, correct = calculate_distance(image, guessed_country)
        ...
        if correct:
            ...
            return JsonResponse({
        "correct": False,
        "message": f"The distance between the center of {guessed_country} and the image coordinates is {distance:.2f} km.",
        "score": score,
