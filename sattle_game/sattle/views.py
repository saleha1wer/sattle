...
def home(request):
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest' and request.POST.get('action') == 'skip':
        ...
        request.session[f'game_state_{user_identifier}']['tries'] = full_tries
...
def view_guesses(request):
    # Fetch only the last 25 guesses
    all_guesses = Guess.objects.all().order_by('-timestamp')[:25]
    ...
    total_guesses_list = [user['total_guesses'] for user in user_stats]
    total_guesses_list.sort()
    length = len(total_guesses_list)
    if length % 2 == 0:
        median_guesses = (total_guesses_list[length // 2 - 1] + total_guesses_list[length // 2]) / 2
    else:
        median_guesses = total_guesses_list[length // 2]

    # Prepare data for the graph
    labels = [user_stat['user_identifier'] for user_stat in user_stats]
    data = [user_stat['total_guesses'] for user_stat in user_stats]
